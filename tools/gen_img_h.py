...
def write_to_h(path, file_name):
    ...
    with open(f"{file_name}.h", 'w') as a:
        ...
        for i in os_list:
            file_name_1 = i.split('.')[0]
            data, size = write_to_h_one(path, file_name_1)
            a.write(data)
            a.write("#define " + file_name_1 + "_size " + str(size) + "\n\n")

        size = len(os_list)
        a.write("const uint8_t *%s[%d] = {" % (file_name, size))
        for i in range(size):
            a.write("%s%d, " % (file_name, i + 1))
        a.write("};\n")
        a.write("const uint32_t %s_size[%d] = {" % (file_name, size))
        for i in range(size):
            a.write("%s%d_size, " % (file_name, i + 1))
        a.write("};\n")
